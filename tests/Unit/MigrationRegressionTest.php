...
/**
 * Migration regression guard for plugin "hypefolders" (Elgg 7.x target).
 *
 * Instantiated from
 * skills/elgg-test-writer/templates/MigrationRegressionTest.php.template and
 * driven by skills/elgg-migrate/references/migration-failure-catalog.md. Every
 * test asserts that a statically-detectable failure CLASS from the catalog is
 * ABSENT for Elgg 7.x.
 *
 * Local deviation from the template: testManifestUsesStringLiterals only flags
 * ::CONST (autoload-triggering) in the entities block, NOT ::class. hypefolders
 * deliberately uses 'class' => MainFolder::class (compile-time constant, no
 * autoload) while keeping 'subtype' a literal string per fix 6466037.
 */
final class MigrationRegressionTest extends TestCase
{
    ...
    /**
     * Bare global constants removed at a major. Keyed cumulatively.
     *
     * @var array<int, list<string>>
     */
     ...
     * or beyond the boundary. Each row: [needle regex, forbidden keyword regex,
     * human message].
     *
    ...
    private static function files(string $sub, string $ext): array
    {
        ...
        foreach ($it as $f) {
            $path = $f->getPathname();
            if (preg_match('#/(vendor|vendors|bower_components|node_modules|mod)/#', $path)) {
                continue;
            }
    ...
    public function testManifestUsesStringLiterals(): void
    {
        ...
        $violations = [];
        // Only ::CONST is dangerous (triggers autoload before the classmap is
        // wired). ::class is a compile-time constant and does not autoload, so
        // 'class' => MainFolder::class is fine and is NOT flagged here.
        if (preg_match('/[\'"](?:class|subtype)[\'"]\s*=>\s*[\\\\\w]+::[A-Z_]{2,}\b/', $block[1])) {
            $violations[] = 'entities block uses ClassName::CONST — use string literals (autoloader not wired at manifest parse time)';
        }
        $this->assertSame([], $violations, "Non-literal class/subtype constant in elgg-plugin.php entities block:\n" . implode("\n", $violations));
    }
